"""
====================================================================
⚡ ENTSO-E Transparency API Downloader — Generation Forecast (A71)
====================================================================
...
📘 DESCRIPTION:
This script downloads **Year-Ahead Generation Forecasts (A71)** from the  
**ENTSO-E Transparency Platform** via its RESTful API.

It retrieves data for all **Nordic bidding zones** (DK1, DK2, FI, NO1–NO5, SE1–SE4)  
for the years **2014–2025**, and outputs a clean, time series CSV file.

Each record includes:
- Bidding zone (e.g., NO3, SE4)
- Production type (Hydro, Wind, Nuclear, etc.)
- Timestamp (UTC)
- Forecasted quantity in MW


--------------------------------------------------------------------
📊 OUTPUT FILE:
The script saves one consolidated CSV file containing all years and zones:

➡️ `nordic_generation_forecast.csv`
...
| Column     | Description |
|-------------|-------------|
| bzn         | Bidding zone name (e.g., NO3, SE4) |
| prodtype    | Production type (e.g., HydroRes, WindOn, Nuclear) |
| timestamp   | Time in UTC (hourly) |
| quantity    | Forecasted value in MW |

--------------------------------------------------------------------
⚙️ PURPOSE:
Used for long-term power system planning and scenario analysis — this script automates  
retrieval of the official **ENTSO-E Year-Ahead Generation Forecast (A71)** per production type.  
====================================================================
"""
...
import time  # Optional: useful for retry logic if needed
...
# Base URL for ENTSO-E Transparency REST API
base_url = "https://web-api.tp.entsoe.eu/api"

# Your ENTSO-E API key
api_key = "0e618bc9-1f36-4186-b70c-b2518618dcaf"

# XML namespace for ENTSO-E data responses
namespaces = {
...
# =============================================================================
# LOAD REFERENCE DATA AND API PARAMETERS
# =============================================================================

# Load bidding zone codes (EICs) from your stored CSV file
areacodes = pd.read_csv('C:/Users/s15832/Documents/Project/Data/entsoe/entsoe_area_codes.csv')

########################################################
# ENTSO-E Parameter Dictionaries (From API Appendix A)
########################################################

# -------------------------------------------------------------------------
# DocumentType: Defines what type of data is requested (A71 = Generation Forecast)
# -------------------------------------------------------------------------
documenttype = pd.DataFrame({
 ...
 'description': [
     ...
     'Generation unavailability', 'Contracted reserves', 'Accepted offers', 'Activated balancing quantities',
     'Activated balancing prices', 'Imbalance prices', 'Imbalance volume', 'Financial situation',
     'Cross border balancing', 'Contracted reserve prices', 'Interconnection network expansion',
     'Counter trade notice', 'Congestion costs', 'DC link capacity', 'Non EU allocations',
     'Configuration document', 'Flow-based allocations', 'Aggregated netted external TSO schedule document',
...
# -------------------------------------------------------------------------
# ProcessType: Defines timeframe or nature of the process (A33 = Year Ahead)
# -------------------------------------------------------------------------
processtype = pd.DataFrame({
 'code': ['A01', 'A02', 'A16', 'A18', 'A31', 'A32', 'A33', 'A39', 'A40', 'A46',
...
# -------------------------------------------------------------------------
# PsrType: Production source codes (e.g., Hydro, Wind, Nuclear)
# -------------------------------------------------------------------------
...
# =============================================================================
# FUNCTIONS: Fetch and Parse ENTSO-E XML Data
# =============================================================================
...
def get_entsoe_data(document_type, process_type, in_domain, period_start, period_end, out_domain=None, bzn=None):
 ...
 if response.status_code == 200:
     ...
     print(f"Response: {response.text[:500]}")  # show partial XML error
...
def parse_xml_response(xml_content, bzn=None):
 """
 Parses XML returned by ENTSO-E API and extracts hourly forecast data.
 """
 ...
 for ts in time_series:
     # Extract the start time of the forecast period
...
# Map psrType codes to human-readable short names (HydroRes, WindOn, etc.)
a71['prodtype'] = a71.psrtype.map(psrtype.reset_index().set_index('code').short_name)

# Save to CSV
a71[['bzn', 'prodtype', 'timestamp', 'quantity']].to_csv(
 'C:/Users/s15832/Documents/Project/Data/entsoe/nordic_generation_forecast.csv',
 index=False
)

print("\n💾 Saved file: nordic_generation_forecast.csv")
print(f"📊 Total records: {len(a71)}")
