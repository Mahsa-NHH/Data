...
Create a CSV file with information on ENTSO-E Transparency area codes
"""

import pandas as pd
import re

# Meaning of area type acronyms (for reference)
acronym_meanings = {
    "BZN": "Bidding Zone",
...
# Area codes and corresponding area types
# From ENTSO-E Transparency restful API documentation (Appendix A)
area_data = {
...
# Initialize the dictionary to collect data for the DataFrame
data = []

# Iterate through the area_data dictionary to populate the DataFrame
for code, description in area_data.items():
    # Create a dictionary for each row
    row = {"code": code}

    # Split the description by comma and iterate through each part
    for entry in description.split(", "):
        if '|' in entry:
            # Split each entry into area type and its corresponding value
            area_type, area_name = entry.split('|')
            row[area_type] = area_name
        else:
            # Treat the remaining part as a full country name
            row["countryarea"] = entry
...
# Reorder columns
columns_order = ["code", "countryarea", "BZN", "BZA", "CTA", "MBA", "IBA", "IPA", "LFA", "LFB", "REG", "SCA", "SNA"]
